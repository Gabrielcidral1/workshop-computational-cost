...
author: "Gabriel Ristow Cidral / Sara Marín López"
date: " 13 / 02 / 20"
output: 
  html_document:
     code_folding: hide
...
system.time(knn_clasif <- knn3(BUILDINGID ~ scale(as.matrix(sample[WAPs])), data = sample))

system.time(knn_clasif_caret<-train(y=sample$BUILDINGID, x=sample[WAPs], data = sample, method="knn"))

# Train two regression knn (with knnreg and caret)
system.time(knn_reg<-knnreg(LONGITUDE ~ as.matrix(sample[WAPs]), data = sample))

system.time(knn_reg_caret<-train(y=sample$LONGITUDE, x=sample[WAPs], data = sample, method="knn"))
...
system.time(svm_clasif <- svm(y = sample$BUILDINGID, x=sample[WAPs]))

system.time(svm_clasif_caret<-train(y=sample$BUILDINGID, x=sample[WAPs], data = sample, method="svmLinear"))

# Train two regression svm (with svm and train)
system.time(svm_reg <- svm(y = sample$LONGITUDE, x=sample[WAPs]))

system.time(svm_reg_caret<-train(y=sample$LONGITUDE, x=as.matrix(sample[WAPs], data = sample, method="svmLinear")))
