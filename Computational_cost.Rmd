...
author: "Gabriel Ristow Cidral / Sara Marín-Lopez"
date: " 13 / 02 / 20"
output: 
  html_document:
    code_folding: hide
...
a <- sample %>% dplyr:: filter(BUILDINGID == i) %>% plot_ly(type = "scatter3d",
        x = ~ LATITUDE,
        y = ~ LONGITUDE,
        z = ~ FLOOR,
        mode = 'markers')
...
for(j in 1:3){
  ...
  a[[j]] <- sample %>% dplyr:: filter(BUILDINGID == i) %>% plot_ly(type = "scatter3d",
        x = ~ LATITUDE,
        y = ~ LONGITUDE,
        z = ~ FLOOR,
        mode = 'markers')
